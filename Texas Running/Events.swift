...
//  Texas Running
//
//  Created by Leon Cai on 8/23/18.
...
struct Item: Codable {
    ...
    let originalStartTime: OriginalStartTime?
    let extendedProperties: ExtendedProperties?
    let transparency: String?
    
    enum CodingKeys: String, CodingKey {
        case kind, etag, id, status, htmlLink, created, updated, summary, creator, organizer, start, end, recurrence, iCalUID, sequence
        case recurringEventID = "recurringEventId"
        case originalStartTime, extendedProperties, transparency
    }
}
...
struct Private: Codable {
    let everyoneDeclinedDismissed: String
}
...
struct OriginalStartTime: Codable {
    let dateTime: Date
    let timeZone: String?
}
